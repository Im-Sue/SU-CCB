...
当前版本：**v1.2.0**。本版重点收口并行需求 closeout 的合并 gate：合法的跨需求 docs、协调件和需求资产不再互相阻塞，但仍不会被错误 stage 或删除；SU-Oriel 同步交付通知收件箱、需求看板分组优化、归档按钮门控与通知消息修复。
...
- 运行环境仅支持 WSL 与 macOS（原生 Windows 走 WSL）—— 见上方「运行依赖与环境」。
